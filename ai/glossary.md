...
weak token :: a configured non-empty viewToken/controlToken shorter than 32 chars is accepted but logged as a startup warning; only empty tokens are auto-generated. Clear the field to rotate to a strong token.
...
view state :: WebSocket client message `{type:"view", pane, session, cols, rows}`; server waits for it before sending the first frame so a reconnecting pinned viewer never flashes the active pane first.
...
lastGoodUrl :: Android's persisted most-recent working endpoint; endpointUrls tries it first, periodically probes saved LAN URLs when on public/ngrok, and may drop a tunnel websocket to rediscover LAN.
